...
use App\Http\Requests\NewCase\PaymentDetailRequest;
...
class PaymentDetailsController extends Controller
{
    ...
    public function paymentDetails(Request $request, $id = null)
    {
        if ($request->ajax()) {
            if ($request->ajax()) {
                $data = $this->paymentDetailService->getCasePaymentDetails($id);
                return DataTables::of($data)
                    ...
                    ->addColumn('payment_mode', function ($row) {
                        return strtoupper($row->payment_mode);
                    })
                    ...
                    ->addColumn('action', function ($row) {
                        ...
                        <a href="javascript:void(0);" data-tableID="payment_detailsTable" data-URL="' . route('delete.court.payment.details', $row->id) . '" class="btn btn-xs btn-danger deletePaymentDetails"><i class="fas fa-trash"></i></a>
    ...
    /**********************************************************************/
    public function storeCourtPaymentDetails(PaymentDetailRequest $request, $id = null)
    {
        ...
        if ($request->ajax()) {
            ...
            try {
                ...
                $saveData = $this->paymentDetailService->store($validatedData, $id);
    ...
    /**********************************************************************/
    public function deleteCourtPaymentDetails(Request $request, $id = null)
    {
        if ($request->ajax()) {
            DB::beginTransaction();
            try {
                $findData = $this->paymentDetailService->findData($id);
                $user = auth()->user();
