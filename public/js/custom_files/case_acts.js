$(document).ready(function () {
    ...
    $(document).on('click', '.update', function (e) {
        ...
        $('#caseActForm').attr('action', url); // Replace '/your-dynamic-action-url' with your dynamic URL


        // Set values to form inputs using jQuery
        $('#caseActForm').find('input, select').each(function () {
    ...
    $(document).on('submit', '#caseActForm', function (e) {
        e.preventDefault();

        let formData = new FormData($('#caseActForm')[0]);
        let btn = $('.caseActSubmitBtn');
        let btnVal = $('.caseActSubmitBtn').text();
        let url = $("#caseActForm").attr('action');
        ...
        $.ajax({
            ...
            beforeSend: function () {
                ...
                if (resp.status === 200) {
                    ...
                    var link = '/case/acts';
                    var currentForm = '.renderCaseActForm';
                    reloadFormComponent(link, currentForm);

                    $('#casActsTable').DataTable().ajax.reload();
